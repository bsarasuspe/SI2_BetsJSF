...
public class HibernateDataAccess implements HibernateDataAccessInterface {
	...
	/*	Question q1;
		...
		session.beginTransaction();
		//List result = session.createQuery("from Event e where e.eventDate = :date").list();
		Date date = UtilDate.newDate(year, month, 17);
		System.out.println(" ");
		System.out.println("Datu-basetik eskuratu gertaerak:");
		Query q = session.createQuery("from Event e where e.eventDate = :date");
		q.setParameter("date", date);
		List result = q.list();
		
        for(int i = 0; i < result.size(); i++) {
            System.out.println(((Event) result.get(i)).getDescription());
        }
